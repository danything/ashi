...
- 世の中の動きは、見出しだけを目にしておく(legs/news.ts、NHK と Yahoo の RSS を 12 時間おき、3 日で忘れる)。見せるのは話すときと X で返すときだけ。**話しながら検索はさせない**(人は会話の途中でネットを引かない。持ち主の指摘、2026-09-26)。目にしなかったことは知らないと言わせる
- 内省の次の一歩に続けて書かれた問いには印(promised)を付ける。2 回以上なら系統が合ったときに先に歩き(reason: promised)、開いた問いの上限で手放さない。歩いたら消す。歩かれなかった問いは、理由(点数の順位・休ませていたテーマ・統合・手放し)を内省に見せる(missedPromises)
- 型の当たり率の基準線(legs/baseline.ts): 1 日 1 回、自己記述を渡さない頭に、自己記述を渡して書いた個性のノート・渡さずに書いた対照のノート・外の文章(参考)を区別なく混ぜて、型が当てはまるかだけを判定させる。主に比べるのは同じ種類の文章の mine と blind(外の文章は文の種類が違い、差の理由を切り分けられなかった)。本数も見せる
- 記憶で言ったことは、頭の申告(unverified)に加えて、足が発言の中の「記憶だけで」「うろ覚え」などの印から拾う(markedClaims)。1 回 8 件まで
