...
- 自己記述は書き直すたびに self-history.jsonl に版を積む。内省には最初の版と、版ごとのぼかし・言い切りの語の数(hedgeStats)を見せる。LLM は書き直すたびにぼかしを削って言い切りへ寄るため
- 内省で頭に自分の型の言葉(patterns)を挙げさせ、walk.json に残す。よそ者との会話で、それが Ashi の最初の返事に出たか・相手が先に言ったかを数える(patternHits)。テーマ名が新しくても中身が自分の型のままのことがある
