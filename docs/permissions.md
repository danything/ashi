...
- **ページが 401 / 403 / 451**(`<host> に弾かれた`): ログインが要るか、ロボットを締め出しているページ。**Ashi は読むだけの足なので、ログインはしない**(鍵を渡す口も作っていない)。要る中身なら「持ち主」の画面で貼り付けるか、公開されている別の出典を話しかけて教える。ロボットの締め出し(公取委・中小企業庁の Akamai など)なら、Ashi が Wayback Machine の写しを探す(`archived_copy`)。読めたらこの知らせは消えるので、残っているのは写しも無かったものだけ
